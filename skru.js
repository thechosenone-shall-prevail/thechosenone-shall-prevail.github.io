...
mashiro_global.ini = new function () {
    this.normalize = function () { // initial functions when page first load (é¦–æ¬¡åŠ è½½é¡µé¢æ—¶çš„åˆå§‹åŒ–å‡½æ•°)
        lazyload();
        social_share();
    ...
    this.pjax = function () { // pjax reload functions (pjax é‡è½½å‡½æ•°)
        pjaxInit();
        social_share();
        post_list_show_animation();
        // copy_code_block();
    ...
    this.change_font = function () {
        if ($("body").hasClass("serif")) {
            ...
            if (document.body.clientWidth <= 860) {
                addComment.createButterbar("Loading fonts from Internet ...");
            }
        }
    }
    this.ini = function () {
...
function attach_image() {
    ...
    $('#upload-img-file').change(function () {
        if (this.files.length > 10) {
            addComment.createButterbar("10 files max per request.");
            return 0;
        }
        for (i = 0; i < this.files.length; i++) {
            if (this.files[i].size >= 5242880) {
                alert('5 MB max per file.\n\nã€Œ' + this.files[i].name + 'ã€\n\nThis image is too large!');
            }
        }
        for (var i = 0; i < this.files.length; i++) {
            ...
            $.ajax({
                ...
                beforeSend: function (xhr) {
                    ...
                    addComment.createButterbar("Uploading...");
                },
                success: function (res) {
                    ...
                    if (res.status == 200) {
                        ...
                        lazyload();
                        addComment.createButterbar("Uploaded successfully!");
                        grin(get_the_url, type = 'Img');
                    } else {
                        addComment.createButterbar("Uploaded failed!<br> Filename: "+f.name+"<br>code: "+res.status+"<br>"+res.message, 3000);
                    }
                },
...
function add_upload_tips() {
    $('<div class="insert-image-tips popup"><i class="fa fa-picture-o" aria-hidden="true"></i><span class="insert-img-popuptext" id="uploadTipPopup">ä¸Šä¼ å›¾ç‰‡</span></div><input id="upload-img-file" type="file" accept="image/*" multiple="multiple" class="insert-image-button">').insertAfter($(".form-submit #submit"));
    attach_image();
...
function copy_code_block() {
    $('pre code').each(function (i, block) {
        ...
        $(this).after('<a class="copy-code" href="javascript:" data-clipboard-target="#hljs-' + i + '" title="æ‹·è´ä»£ç "><i class="fa fa-clipboard" aria-hidden="true"></i></a>');
    });
...
$(document).on("click", ".sm", function () {
    var msg = "Are you sure to set this private?";
    if (confirm(msg) == true) {
        $(this).commentPrivate();
    } else {
        alert("Cancelled");
...
$.fn.commentPrivate = function () {
    if ($(this).hasClass('private_now')) {
        alert('You have already set this private before.');
        return false;
    } else {
...
function add_copyright() {
    ...
    function setClipboardText(event) {
        ...
        var htmlData = "# For commercial use, please contact the author for authorization. For non-commercial use, please indicate the source.<br>" + "# Licens: CC BY-NC-SA 4.0<br>" + "# Author: " + mashiro_option.author_name + "<br>" + "# URL: " + window.location.href + "<br>" + "# Source: " + mashiro_option.site_name + "<br><br>" + window.getSelection().toString().replace(/\r\n/g, "<br>");;
            ...
            $('.js-toggle-search').on('click', function () {
                ...
                if (mashiro_option.live_search) {
                    ...
                    function query(B, A, z) {
                        ...
                        w && (y = y + G + "æ–‡ç« " + E + w + D), u && (y = y + G + "é¡µé¢" + E + u + D), r && (y = y + G + "åˆ†ç±»" + E + r + D), p && (y = y + G + "æ ‡ç­¾" + E + p + D), F && (y = y + G + "è¯„è®º" + E + F + D), s = addComment.I("PostlistBox"), s.innerHTML = y
                    }
                }
            });
        ...
        XLS: function () {
            ...
            var intersectionObserver = new IntersectionObserver(function (entries) {
                ...
                if (page_next != undefined && load_key) {
                    ...
                    if (load_time != "233") {
                        console.log("%c Automatic loading countdown %c", "background:#9a9da2; color:#ffffff; border-radius:4px;", "", "", load_time);
        ...
        XCS: function () {
            ...
            jQuery(document).on("submit", "#commentform", function () {
                jQuery.ajax({
                    ...
                    beforeSend: addComment.createButterbar("Submiting ..."),
...
$(function () {
    ...
    if (Poi.pjax) {
        $(document).pjax('a[target!=_top]', '#page', {
            fragment: '#page',
            timeout: 8000,
        }).on('pjax:beforeSend', () => { //ç¦»å¼€é¡µé¢åœæ­¢æ’­æ”¾
