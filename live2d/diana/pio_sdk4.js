...
function loadlive2d(canvas_id, json_object_or_url, on_load) {
    ...
    console.log("[Pio] Checking dependencies...")
    console.log("[Pio] PIXI available:", typeof PIXI !== 'undefined')
    console.log("[Pio] PIXI.live2d available:", typeof PIXI.live2d !== 'undefined')
    
    // Check if PIXI.live2d is available
    if (typeof PIXI === 'undefined' || typeof PIXI.live2d === 'undefined') {
        console.error("[Pio] PIXI.live2d is not loaded!")
        return
    }

    // Find the Live2DModel class
    var Live2DModel = PIXI.live2d.Live2DModel || window.PIXI.Live2DModel;
    
    // Check all properties to find the model loader
    if (!Live2DModel) {
        console.log("[Pio] Searching for Live2DModel in PIXI.live2d...");
        var keys = Object.keys(PIXI.live2d);
        console.log("[Pio] Available properties:", keys);
        
        // Try to find it by checking each property
        for (var i = 0; i < keys.length; i++) {
            if (typeof PIXI.live2d[keys[i]] === 'function' || typeof PIXI.live2d[keys[i]].from === 'function') {
                Live2DModel = PIXI.live2d[keys[i]];
                console.log("[Pio] Found model loader at PIXI.live2d." + keys[i]);
                break;
            }
        }
    }
    ...
    if (!Live2DModel || typeof Live2DModel.from !== 'function') {
        console.error("[Pio] Could not find Live2DModel.from() method!");
        console.log("[Pio] PIXI.live2d contents:", PIXI.live2d);
        return;
    }
    ...
    // Use the async method instead of fromSync
    console.log("[Pio] Loading model from:", json_object_or_url)
    
    Live2DModel.from(json_object_or_url).then(model => {
        console.log("[Pio] Model loaded successfully!")
        ...
        console.error("[Pio] Failed to load model:", error)
    })
...
function _pio_initialize_pixi() {
    ...
    // Container already exists in HTML, so we don't need to create it
